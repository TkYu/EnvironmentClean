            ...
            if (IsAdmin)
            {
                ...
                await Task.Run(() =>
                {
                    ...
                    if (ue != null && me != null)
                    {
                        ...
                        var mp = ue.GetValue("Path", null, RegistryValueOptions.DoNotExpandEnvironmentNames).ToString();
                        var mProcess = Process(up.Split(';'));
                        if (mp != mProcess)
                        {
                            machine = mp;
                            me.SetValue("Path",uProcess,RegistryValueKind.String);
                        }
